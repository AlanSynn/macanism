...
# macanism = macanism(vectors=(a, b, c, d, e, x, y), input_vector=a, guess=(guess1, guess2, guess3), pos=0,
#                       vel=-50*np.pi, acc=0, loops=loops)
# macanism.calculate()
# macanism.tables(position=True, velocity=True, acceleration=True)
# macanism.plot(velocity=True, acceleration=True)
...
macanism = macanism(vectors=(a, b, c, d, e, x, y), origin=O, guess=(guess1, guess2, guess3), pos=t2_t,
                      vel=w2_t, acc=a2_t, loops=loops)
macanism.iterate()
ani, fig, ax = macanism.get_animation(velocity=True, acceleration=True, stamp=time, stamp_loc=(0.5, 0.9), cushion=0.5)
