...
# Define the macanism
macanism = macanism(vectors=[a, b, c, d], origin=O, loops=loop, pos=th2, vel=w2, acc=a2,
                      guess=(pos_guess, vel_guess, acc_guess))
macanism.iterate()  # solves for each item in the defined input arrays


# Get the animation, figure, and axes objects
ani, fig, ax = macanism.get_animation(velocity=True, acceleration=True, scale=0.2)
