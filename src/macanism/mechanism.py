...
class macanism:
    ...
    def tables(self, position=False, velocity=False, acceleration=False, to_five=False):
        ...
        if position:
            ...
            print('--------\n')
            if not to_five:
                macanism_data = [[v, v.r, np.rad2deg(v.theta), v.x, v.y] for v in self.positions]
                joint_data = [[j, j.x_pos, j.y_pos] for j in sorted(self.joints, key=lambda x: x.name)]
            else:
                macanism_data = [[v, f'{v.r:.5f}', f'{np.rad2deg(v.theta):.5f}', f'{v.x:.5f}', f'{v.y:.5f}'] for v
                                  in self.positions]
                joint_data = [[j, f'{j.x_pos:.5f}', f'{j.y_pos:.5f}'] for j in
                              sorted(self.joints, key=lambda x: x.name)]
            Data(macanism_data, headers=['Vector', 'R', 'Theta', 'x', 'y']).print(table=True)
            print('')
        ...
        if velocity:
            ...
            print('--------\n')
            if not to_five:
                macanism_data = [[v, v._get_mag()[0], np.rad2deg(v._get_mag()[1]), v.x, v.y] for v in
                                  self.velocities]
                ...
                macanism_data = [[v, f'{v._get_mag()[0]:.5f}', f'{np.rad2deg(v._get_mag()[1]):.5f}', f'{v.x:.5f}',
            ...
            Data(macanism_data, headers=['Vector', 'Mag', 'Angle', 'x', 'y']).print(table=True)
            print('')
    ...
    def plot(self, velocity=False, acceleration=False, scale=0.1, show_joints=True, grid=True, cushion=1):
        """
        Plots the instance of the macanism; calculate() method must be called before calling this method.
    ...
    def calculate(self):
        ...
        vectors and joints if vel and acc for the macanism is given.
        """
        for v in self.vectors:
            v.get = v.pos.get
    ...
    def get_bounds(self):
        """
        :return: Two tuples; the first is the minimum and maximum x position of the macanism, and the second is the
            minimum and maximum y position of the macanism.
        """
